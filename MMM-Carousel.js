...
Module.register('MMM-Carousel', {
    ...
    validKeyPress: function (kp) {
        if (kp.keyName === this.keyHandler.config.map.NextSlide) {
            ...
            //this.restartTimer();
            this.sendSocketNotification("CAROUSEL_NEXT", null);
        } else if (kp.keyName === this.keyHandler.config.map.PrevSlide) {
            //this.manualTransition(undefined, -1);
            //this.restartTimer();
            this.sendSocketNotification("CAROUSEL_PREVIOUS", null);
            ...
            if (typeof parseInt(goToSlide[1]) === "number") {
                ...
                //this.restartTimer();
                this.sendSocketNotification("CAROUSEL_GOTO", goToSlide[1] + 1);
            }
        }
    },
    ...
    setUpTransitionTimers: function (positionIndex, goToIndex) {
        var self = this;
        ...
        modules.writeIndex = this.writeIndex.bind(this);
        modules.nextPage=this.nextPage.bind(this);
    ...
    writeIndex: function (index) {
        this.sendSocketNotification("INDEX", { index: index });
    },

    nextPage: function (index) {
        this.sendSocketNotification("CAROUSEL_GOTO", index + 1);
    },
    ...
    restartTimer: function () {
        if (this.config.transitionInterval > 0 && this.config.setTimer) {
            // Restart the timer
    ...
    manualTransitionCallback: function (slideNum) {
        ...
        //this.restartTimer();
        self.sendSocketNotification("CAROUSEL_GOTO", slideNum + 1)
    },
    ...
    getDom: function () {
        ...
        function makeOnChangeHandler(id) {
            return function () {
                that.manualTransitionCallback(id);
            };
        }
    ...
    socketNotificationReceived: function (noti, payload) {
        if (noti == "INITA") {
            ...
            if ((this.config.mode === 'global') || (this.config.mode === 'slides')) {
                this.setUpTransitionTimers(null, payload.index);
            } else {
                for (position = 0; position < positions.length; position += 1) {
                    if (this.config[positions[position]].enabled === true) {
                        this.setUpTransitionTimers(positions[position], payload.index);
                    }
                }
            }
